...
<a name="tab1"></a>
# JAVASCRIPT
`null и undefined` - это два различных значения, которые могут присваиваться переменным в JavaScript.

null означает отсутствие значения и используется, когда значение явно должно быть пустым. Например, когда переменная не имеет ссылки на объект, можно ей присвоить значение null. Также null может использоваться для явного указания на то, что функция не возвращает никакого значения.

undefined означает, что значение переменной не было определено. Когда переменная объявлена, но ей не было присвоено никакого значения, её значение будет undefined. Также undefined может быть возвращено, когда функция не содержит оператора return.

Различия между null и undefined:

null - это явно определенное значение, которое указывает на отсутствие значения, а undefined указывает на то, что значение не определено.
null - это объект, а undefined - это примитивный тип данных.
Если вы пытаетесь получить доступ к свойству или методу переменной со значением null, JavaScript не выдаст ошибку, но если вы попытаетесь получить доступ к свойству или методу переменной со значением undefined, JavaScript выдаст ошибку.
...
## 
`AJAX` - (Asynchronous JavaScript and XML) - это техника веб-разработки, которая позволяет обновлять содержимое веб-страницы динамически без перезагрузки всей страницы. Она основана на нескольких технологиях, включая HTML, CSS, JavaScript и XML или JSON.

Основная идея AJAX заключается в использовании JavaScript для создания асинхронных запросов к веб-странице, которая обновляет только необходимую часть содержимого. Это позволяет улучшить пользовательский опыт и уменьшить нагрузку на сервер. Примеры использования AJAX включают динамическую загрузку новостей, обновление списка товаров без перезагрузки страницы, отправку формы без перезагрузки страницы и т.д.
...
##
`arguments` - это псевдомассив, доступный внутри каждой функции JavaScript, который содержит значения аргументов, переданных функции при её вызове.

arguments позволяет обращаться к аргументам функции, даже если они не были определены в списке параметров функции. Также можно получить доступ к arguments.length, чтобы узнать количество аргументов, переданных функции.
...
- ES6 (ES2015): шаблонные строки, стрелочные функции, классы, блочная область видимости переменных (let и const), деструктуризация, параметры по умолчанию, оператор разворота (...), модули, итераторы и генераторы;
